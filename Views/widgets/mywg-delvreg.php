...
select ifnull(sum(TQTY),0) TQTY,ifnull(SUM(TCOST),0) TCOST,ifnull(SUM(TSRP),0) TSRP,max(MLRCVDDTE) MLRCVDDTE,count(mat_code) nitem_cnt,count(*) nrec_cnt  from (
	...
	WHERE aa.`flag` = 'R'
	AND (aa.`hd_sm_tags` ='D' OR aa.`hd_sm_tags` ='S' OR aa.`hd_sm_tags` ='M' OR aa.`hd_sm_tags` ='C')  and aa.branch_id = '$mbrnch_id' 
...
if($q->getNumRows() > 0) { 
	...
	//echo "<h6>Qty: " . number_format($rw['TQTY'],2,'.',',') . "</h6>";
	//echo "<h6>Tot. SKU: " . number_format($rw['nitem_cnt'],2,'.',',') . "</h6>";
	//echo "<h6>Tot. Amount: " . number_format($rw['TSRP'],2,'.',',') . "</h6>";
	//echo  "<span>Last Received: {$MLDELVDTE}</span>";
}
$q->freeResult();
