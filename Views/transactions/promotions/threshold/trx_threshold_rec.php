			...
			<tbody>
				...
				if($rlist !== ''):
					...
					foreach($rlist as $row): 
						...
						$threshold_trxno = ($row['threshold_trxno']);
						$dis = ($row['is_approved'] == 'N' ? "disabled" : '');
						
						?>
						<tr bgcolor="<?=$bgcolor;?>" <?=$on_mouse;?>>
							<td class="text-center" nowrap>
								<?=anchor('me-threshold-vw/?threshold_trxno=' . $threshold_trxno, '<i class="bi bi bi-pencil" style="background-color:#167F92; color:#fff; padding: 3px 5px 3px;; border-radius: 5px;"></i>',' class="btn btn-dgreen p-1 pb-0 mebtnpt1 btn-sm"');?>
								
							</td>
							<td nowrap><?=$row['threshold_trxno'];?></td>
							...
							<td>
								<button class="btn btn-success btn-xs" onclick="javascript:__mbtn_promo_bdownload('<?=$row['threshold_trxno'];?>');" <?=$dis;?>> <i class="bi bi-print"></i> Download</button>
							</td>
...
<script type="text/javascript"> 
	function __myredirected_rsearch(mobj) { 
		try { 
			...
			jQuery.ajax({ // default declaration of ajax parameters
				type: "POST",
				url: '<?=site_url();?>me-threshold-recs',
    ...
    function __mbtn_promo_bdownload(threshold_trxno){
    	try { 
    		__mysys_apps.mepreloader('mepreloaderme',true);
    		...
    		var mparam = {
    			threshold_trxno: threshold_trxno
                  ...
                  jQuery.ajax({ // default declaration of ajax parameters
                  	type: "POST",
                  	url: '<?=site_url();?>me-threshold-barcode-dl',
                  	context: document.body,
                  	data: eval(mparam),
                  	global: false,
                  	cache: false,
