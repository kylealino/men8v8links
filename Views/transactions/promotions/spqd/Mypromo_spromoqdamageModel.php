...
class Mypromo_spromoqdamageModel extends Model
{
        ...
        if(count($adata1) > 0) { 
            ...
            for($aa = 0; $aa < count($adata1); $aa++) { 
                ...
                if(!empty($mtkn_mntr) && $getrw == 0) {  
                    $isValid=$this->isBelowCost($adata1,$art_ucost,$arrayIndex);
                    if ($isValid){
                        $valid = 1;
                        $str2 = "
                ...
                if(!empty($mtkn_mntr)) { 
                    $isValid=$this->isBelowCost($adata1,$art_ucost,6);
                    if ($isValid){
                        $valid = 1;
    ...
    public function spqd_for_approval() {
       ...
       foreach ($q->getResultArray() as $data) {
           ...
           $this->save_promo_spqd_from_POS2($spqd_trxno, $B_CODE_POS, $adataApproval, $txt_spromo);
       }
       for($aa = 0; $aa < count($adata1); $aa++) { 
    ...
    public function save_promo_spqd_from_POS2($spqd_trxno,$B_CODE_POS, $adataApproval,$txt_spromo){
           ...
           if ($this->myposdbconn):
               $str_items = '';
               for ($aa = 0; $aa < count($adataApproval); $aa++) :
                   $medata = $adataApproval[$aa];
                 
                   $qty = trim($medata['qty']);
                   $new_srp = trim($medata['new_srp']);
                   $mitemc = trim($medata['mitemc']);
                   $startDate = trim($medata['startDate']);
                   $startTime = trim($medata['startTime']);
                   $endDate = trim($medata['endDate']);
                   $endTime = trim($medata['endTime']);
              ...
              if ($promoid > 0 && $brid > 0):  //validate promo id and branch id
				...
				if ($row_count > 0):
					...
					sqlsrv_free_stmt( $stmt);
                        ...
                        $mevalarray = array(1,$meproditem,$brid);
                        $stmt1 = sqlsrv_query( $this->myposdbconn, $str1,$mevalarray, array("Scrollable"=>"buffered") );
                        if( $stmt1 === false) { 
                            die( print_r( sqlsrv_errors(), true) . chr(13) . chr(10) . 'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
                        }
                        ...
                        if ($row_count > 0):
                            ...
                            ";
                            $mevalarray = array($promoidbr,$promoid,$meproditem,$qty,0);
                            $stmt = sqlsrv_query( $this->myposdbconn, $str,$mevalarray, array("Scrollable"=>"buffered") );
