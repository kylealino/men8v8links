			...
			<tbody>
				...
				if($rlist !== ''):
					...
					foreach($rlist as $row): 
						...
						$mtkn_attr = hash('sha384', $row['recid'] . $mpw_tkn);
						...
						<tr bgcolor="<?=$bgcolor;?>" <?=$on_mouse;?>>
							<td class="text-center" nowrap>
								<?=anchor('me-promo-vw/?mtkn_mntr=' . $mtkn_attr, '<i class="bi bi bi-pencil"></i>',' class="btn btn-dgreen p-1 pb-0 mebtnpt1 btn-sm"');?>
							</td>
							...
							<td nowrap><?=$row['is_approved'];?></td>
							<td nowrap><?= $mylibzsys->mydate_mmddyyyy($row['date_approved']);?></td>
							<td>
								<button class="btn btn-success btn-sm" onclick="javascript:__mbtn_promo_bdownload('<?=$row['promo_trxno'];?>');" <?=$dis;?>> <i class="bi bi-print"></i> Download</button>
							</td>
						</tr>
						<?php 
						$nn++;
