			...
			<tbody>
				...
				if($rlist !== ''):
					...
					foreach($rlist as $row): 
						...
						$buyanyatprice_trxno = ($row['buyanyxatpricey_trxno']);
						$dis = ($row['is_approved'] == 'N' ? "disabled" : '');
						
						?>
						<tr bgcolor="<?=$bgcolor;?>" <?=$on_mouse;?>>
							<td class="text-center" nowrap>
								<?=anchor('me-buyanyatprice/?buyanyxatpricey_trxno=' . $buyanyatprice_trxno, '<i class="bi bi bi-pencil" style="background-color:#167F92; color:#fff; padding: 3px 5px 3px;; border-radius: 5px;"></i>',' class="btn btn-dgreen p-1 pb-0 mebtnpt1 btn-sm"');?>
								
							</td>
							<td nowrap><?=$row['buyanyxatpricey_trxno'];?></td>
							...
							<td nowrap><?=$row['promo_name'];?></td>
                            <td nowrap><?=$row['quantity'];?></td>
                            <td nowrap><?=$row['price'];?></td>
							<td nowrap><?= $mylibzsys->mydate_mmddyyyy($row['encd_date']);?></td>

							<td>
								<button class="btn btn-success btn-xs" onclick="javascript:__mbtn_buy1take1_bdownload('<?=$row['buyanyxatpricey_trxno'];?>');" <?=$dis;?>> <i class="bi bi-print"></i> Download</button>
							</td>
...
<script type="text/javascript"> 
	function __myredirected_rsearch(mobj) { 
		try { 
			...
			jQuery.ajax({ // default declaration of ajax parameters
				type: "POST",
				url: '<?=site_url();?>me-buyanyatprice-recs',
    ...
    function __mbtn_buy1take1_bdownload(buyanyatprice_trxno){
    	try { 
    		__mysys_apps.mepreloader('mepreloaderme',true);
    		
    		var mparam = {
    			buyanyatprice_trxno: buyanyatprice_trxno

    		}; 
                   //console.log(mparam);
                  jQuery.ajax({ // default declaration of ajax parameters
                  	type: "POST",
                  	url: '<?=site_url();?>me-buyanyatprice-barcode-dl',
                  	context: document.body,
                  	data: eval(mparam),
                  	global: false,
                  	cache: false,
