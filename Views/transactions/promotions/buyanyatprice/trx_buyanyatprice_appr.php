			...
			<tbody>
				...
				if($rlist !== ''):
					...
					<tr bgcolor="<?=$bgcolor;?>" <?=$on_mouse;?>>
						<td>
							<button class="btn btn-dgreen btn-sm" Style="background-color: #167F92; color: #FFF; padding: 2px 6px 2px 6px; " onclick="javascript:save_for_approve_buyanyatprice('<?=$mtkn_recid;?>');" <?=$dis;?>> <i class="bi bi-print"></i> For Approved</button>
						</td>
...
<script type="text/javascript"> 
	...
	function __myredirected_rsearch_appr(mobj) { 
		try { 
			...
			type: "POST",
			url: '<?=site_url();?>me-buyanyatprice-appr',
	...
	function save_for_approve_buyanyatprice(mtkn_recid){
		try { 
            __mysys_apps.mepreloader('mepreloaderme',true);
                  ...
                  jQuery.ajax({ // default declaration of ajax parameters
                    type: "POST",
                    url: '<?=site_url();?>me-buyanyatprice-appr-save',
                    context: document.body,
                    data: eval(mparam),
                    global: false,
                    cache: false,
