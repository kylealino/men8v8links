			...
			<tbody>
				...
				if($rlist !== ''):
					...
					foreach($rlist as $row): 
						...
						$buy1take1_trxno = ($row['buy1take1_trxno']);
						$dis = ($row['is_approved'] == 'N' ? "disabled" : '');
						
						?>
						<tr bgcolor="<?=$bgcolor;?>" <?=$on_mouse;?>>
							<td class="text-center" nowrap>
								<?=anchor('me-buy1take1-vw/?buy1take1_trxno=' . $buy1take1_trxno, '<i class="bi bi bi-pencil" style="background-color:#167F92; color:#fff; padding: 3px 5px 3px;; border-radius: 5px;"></i>',' class="btn btn-dgreen p-1 pb-0 mebtnpt1 btn-sm"');?>
								
							</td>
							<td nowrap><?=$row['buy1take1_trxno'];?></td>
							...
							<td>
								<button class="btn btn-success btn-xs" onclick="javascript:__mbtn_buy1take1_bdownload('<?=$row['buy1take1_trxno'];?>');" <?=$dis;?>> <i class="bi bi-print"></i> Download</button>
							</td>
...
<script type="text/javascript"> 
	function __myredirected_rsearch(mobj) { 
		try { 
			...
			jQuery.ajax({ // default declaration of ajax parameters
				type: "POST",
				url: '<?=site_url();?>me-buy1take1-recs',
    ...
    function __mbtn_buy1take1_bdownload(buy1take1_trxno){
    	try { 
    		__mysys_apps.mepreloader('mepreloaderme',true);
    		
    		var mparam = {
    			buy1take1_trxno: buy1take1_trxno

    		}; 
                   //console.log(mparam);
                  jQuery.ajax({ // default declaration of ajax parameters
                  	type: "POST",
                  	url: '<?=site_url();?>me-buy1take1-barcode-dl',
                  	context: document.body,
                  	data: eval(mparam),
                  	global: false,
                  	cache: false,
