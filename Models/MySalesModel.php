    ...
    public function download_buy1take1_barcode($buy1take1_trxno){
        ...
        if($qry->resultID->num_rows == 0) { 
            ...
            FROM(
                ...
                SELECT
                CONCAT('E',a.`branch_code`),CONCAT(b.`buyxtakey_trxno`,'-',ROW_NUMBER() OVER (ORDER BY a.`buyxtakey_trxno`)),'BUY X TAKE Y', CONCAT(a.`start_date`,' ', a.`start_time`),CONCAT(a.`end_date`,' ', a.`end_time`),b.`qty`, b.`take`, a.`is_discount_percent`,a.`is_discount_amount`,
      ...
      if((!empty($branch_code)) && !empty($mtkn_branch)) {
        ...
        if(!empty($mtkn_mntr)) { 
            ...
            //generate voucher transaction number
                ...
                $voucher_trxno =  $this->mydataz->get_ctr_promotions('PV','',$this->db_erp,'CTRL_NO03');//VOUCHER TRANSACTION NUMBER
    ...
    public function download_voucher_barcode($voucher_trxno){
        ...
        if($voucher_trxno != ''){
            ...
            FROM(
                ...
                SELECT
                CONCAT('E',a.`branch_code`),a.`voucher_trxno`,'500 off',CONCAT(a.`start_date`,' ',a.`start_time`),CONCAT (a.`end_date`,' ',a.`end_time`),
            ...
            if(count($adata1) > 0) { 
                ...
                for($aa = 0; $aa < count($adata1); $aa++) { 
                    ...
                    if(!empty($ART_CODE)) { 
                        ...
                        if ($cb_fix_percent_discount_value == 1) {
                                if ($discount_value >= $invalid_disc) {
                                    echo "<div class=\"alert alert-danger\" role=\"alert\"><strong>Info.<br/></strong><strong>Error</strong> Discount value must not be greater than 75%</div>";
                                    die();
                                }
                            }
    ...
    public function download_wholesale_barcode($wholesale_trxno){
        ...
        if($wholesale_trxno != ''){
            ...
            FROM(
                ...
                SELECT
                CONCAT('E',a.`branch_code`),a.`wholesale_trxno`,CONCAT ('avail ',`quantity`,' ','Items ','get ',`discount_value`,'%'),CONCAT (a.`start_date`,' ',a.`start_time`),CONCAT (a.`end_date`,' ',a.`end_time`),
