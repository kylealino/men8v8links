...
class MyPromoBuy1take1Model extends Model
{
        ...
        if(!empty($mtkn_mntr)) { 
            ...
            if(count($adata1) > 0) { 
                ...
                for($aa = 0; $aa < count($adata1); $aa++) { 
					...
					JOIN {$this->db_erp}.`trx_pos_promo_buy1take1_dt` b  WHERE a.`branch_code` = '$B_CODE' 
    ...
    public function save_promo_buy1take1_from_POS($buy1take1_trxno,$promo_name,$cb_fix_value,$qty,$take,$B_CODE_POS,$adata1,$start_date,$start_time,$end_date,$end_time){
		...
		if ($this->myposdbconn):
			...
			$promoid = 0;
			$str = "select [id] from [diQtech_db].[dbo].[diQt_PromoBuyXTakeY] where [code] = ?";
			$stmt = sqlsrv_query( $this->myposdbconn, $str,array($buy1take1_trxno), array("Scrollable"=>"buffered") );
			if( $stmt === false) { 
				die( print_r( sqlsrv_errors(), true) . chr(13) . chr(10) . 'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
			}
			$row_count = sqlsrv_num_rows( $stmt );
			if ($row_count > 0):
				...
				$is_fixed_price,$meproditems,$meproditems,$promoid);
				...
				if($row_count > 0) {
					...
					$str = "
					insert into [diQtech_db].[dbo].[diQt_PromoBuyXTakeY] ([id]
					,[code]
					,[name]
					,[start_date]
					,[end_date]
					...
					$mend_date = $end_date . ' ' . $end_time;
					$is_fixed_price = 1;
					$mevalarray = array($promoid,$buy1take1_trxno,$promo_name,$mstart_date,$mend_date,
					$qty,$take,$is_fixed_price,$meproditems,$meproditems,0);
					$stmt = sqlsrv_query( $this->myposdbconn, $str,$mevalarray, array("Scrollable"=>"buffered") );
					if( $stmt === false) { 
						die( print_r( sqlsrv_errors(), true) . chr(13) . chr(10) . 'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
					}
