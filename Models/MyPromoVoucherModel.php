...
class MyPromoVoucherModel extends Model
{
    ...
    public function voucher_entry_save() { 
      ...
      if((!empty($branch_name)) && !empty($mtkn_branch)) {
        ...
        if(!empty($mtkn_mntr)) { 
            ...
            $str = "select aa.`recid`,aa.`voucher_trxno` from {$this->db_erp}.`gw_voucher_hd` aa where voucher_trxno = '$mtkn_mntr'";
            ...
            if(count($adata1) > 0) { 
                ...
                for($aa = 0; $aa < count($adata1); $aa++) { 
                    ...
                    if(empty($discount_value)){
                        echo "<div class=\"alert alert-danger mb-0\" role=\"alert\"><strong>Info.<br/></strong><strong>Error</strong> Please enter a Voucher Amount or Discount.</div>";
                        die(); 
                    ...
                    if(empty($voucher_code)){
                        ...
                        die(); 
                            ...
                            AND b.voucher_code REGEXP CONCAT('[[:<:]]', '$voucher_code', '[[:>:]]')
                             and 
                ...
                if(count($adata1) > 0) { 
                    ...
                    for($xx = 0; $xx < count($adatar1); $xx++) { 
                        ...
                        $voucher_code2 = $xdata[1];
                     

                        $str1 = "
                        SELECT `voucher_code`,  `voucher_trxno` FROM {$this->db_erp}.`gw_voucher_dt` WHERE `voucher_trxno` = '$voucher_trxno' AND `voucher_code` = '$voucher_code2' 
                        ...
                        if(!empty($mtkn_mntr)){ 
                            ...
                            update {$this->db_erp}.`gw_voucher_dt` set 
                            `voucher_trxno` = '$voucher_trxno',
                            `amount` = '$discount_value',
                            `voucher_code` = '$voucher_code',
                            `is_disabled` = '$is_disabled'
                            where voucher_trxno = '$voucher_trxno' and voucher_code = '$voucher_code2'

                            ";
                            $q1 = $this->mylibzdb->myoa_sql_exec($str3,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__); 
                        }   
                    ...
                    //show success modal
                    if(empty($mtkn_mntr)) { 
                        $this->save_promo_voucher_from_POS($voucher_trxno,$voucher_name,$voucher_code,$voucher_amount,$discount_value,$B_CODE_POS,$adata1,$start_date,$start_time,$end_date,$end_time);
    ...
    public function save_promo_voucher_from_POS($voucher_trxno,$voucher_name,$voucher_code,$voucher_amount,$discount_value,$B_CODE_POS,$adata1,$start_date,$start_time,$end_date,$end_time){
		$lcon = 0;
		...
		if ($this->myposdbconn):
			...
			if ($row_count > 0):
				...
				$mevalarray = array($voucher_name,$mstart_date,$mend_date,$voucher_amount,$discount_value,
				$promoid);
				...
				if($row_count > 0) {
					...
					$voucher_amount,$discount_value,0);
					$stmt = sqlsrv_query( $this->myposdbconn, $str,$mevalarray, array("Scrollable"=>"buffered") );
					if( $stmt === false) { 
						die( print_r( sqlsrv_errors(), true) . chr(13) . chr(10) . 'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
					}
			...
			//branch detail table promo fixed/discount 
             ...
             if ($promoid > 0 && $brid > 0):  //validate promo id and branch id
                $voucher_codes_array = explode(",", $voucher_codes);
              
             
                foreach ($voucher_codes_array as $voucher) {
				$str = "select [id] from [diQtech_db].[dbo].[diQt_PromoVoucherCode] where [promo_voucher_id] = ? and 
				[code] = ?";
				$stmt = sqlsrv_query( $this->myposdbconn, $str,array($promoid,$voucher), array("Scrollable"=>"buffered") );
				if ( $stmt === false):
					die( print_r( sqlsrv_errors(), true) . chr(13) . chr(10) . 'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__ );
				endif;
				$row_count = sqlsrv_num_rows( $stmt );
				...
				if ($row_count > 0):
					...
					if ($row_count > 0):
                        ...
                        $str = "insert into [diQtech_db].[dbo].[diQt_PromoVoucherCode] ([id], [promo_voucher_id], [code], [used], [date_changed], [disable]) values (?,?,?,?,getdate(),?)";
            
                        $mevalarray = array($promoidbr, $promoid, $voucher, 0, 0);
                        $stmt_insert = sqlsrv_query($this->myposdbconn, $str, $mevalarray, array("Scrollable" => "buffered"));
